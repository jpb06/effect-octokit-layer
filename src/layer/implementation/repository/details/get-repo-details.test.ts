...
describe('getRepoDetails effect', () => {
  const owner = 'yolo';
  const repo = 'bro';
  ...
  it('should return data with links', async () => {
    ...
    const { getRepoDetails } = await import('./get-repo-details.js');

    const task = getRepoDetails({ owner, repo });
  ...
  it('should fail with an Octokit request error', async () => {
    ...
    const task = pipe(getRepoDetails({ owner, repo }), Effect.flip);
    const result = await Effect.runPromise(task);
  ...
  it('should fail if an api rate limit error', async () => {
    ...
    const task = pipe(getRepoDetails({ owner, repo }), ConsoleTestLayer);
