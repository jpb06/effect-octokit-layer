...
import type { GetRepoReleasesPageArgs } from './get-repo-releases-page.js';

vi.mock('@octokit/core');

describe('getRepoReleasesPage effect', () => {
  const args: GetRepoReleasesPageArgs = {
    owner: 'cool',
    repo: 'story bro',
    page: 1,
  };
  ...
  it('should fail if github token env variable is not set', async () => {
    ...
    const { getRepoReleasesPage } = await import('./get-repo-releases-page.js');

    const task = pipe(getRepoReleasesPage(args), Effect.flip);
  ...
  it('should return data with links', async () => {
    ...
    const task = getRepoReleasesPage(args);
  ...
  it('should fail with an api rate limit error', async () => {
    ...
    const task = pipe(getRepoReleasesPage(args), ConsoleTestLayer);
