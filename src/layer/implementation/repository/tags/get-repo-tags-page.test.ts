...
import type { GetRepoTagsPageArgs } from './get-repo-tags-page.js';

vi.mock('@octokit/core');

describe('getRepoTagsPage effect', () => {
  const args: GetRepoTagsPageArgs = {
    owner: 'cool',
    repo: 'story bro',
    page: 1,
  };
  ...
  it('should fail if github token env variable is not set', async () => {
    ...
    const { getRepoTagsPage } = await import('./get-repo-tags-page.js');

    const task = pipe(getRepoTagsPage(args), Effect.flip);
  ...
  it('should return data with links', async () => {
    ...
    const task = getRepoTagsPage(args);
  ...
  it('should fail with an api rate limit error', async () => {
    ...
    const task = pipe(getRepoTagsPage(args), ConsoleTestLayer);
