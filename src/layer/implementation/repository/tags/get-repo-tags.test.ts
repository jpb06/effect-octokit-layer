...
import type { GetRepoTagsArgs } from './get-repo-tags.js';

vi.mock('@octokit/core');

describe('getRepoTags effect', () => {
  const args: GetRepoTagsArgs = {
    owner: 'cool',
    repo: 'story bro',
  };
  ...
  it('should retun multiple pages data', async () => {
    ...
    const { getRepoTags } = await import('./get-repo-tags.js');

    const task = getRepoTags(args);
  ...
  it('should fail when one request fails', async () => {
    ...
    const task = pipe(getRepoTags(args), Effect.flip);
    const result = await Effect.runPromise(task);
