...
import type { GetIssueCommentsPageArgs } from './get-issue-comments-page.js';

vi.mock('@octokit/core');

describe('getIssueCommentsPage effect', () => {
  const args: GetIssueCommentsPageArgs = {
    owner: 'cool',
    repo: 'story bro',
    issueNumber: 1,
    page: 1,
  ...
  it('should return data with links', async () => {
    ...
    const task = getIssueCommentsPage(args);
  ...
  it('should fail with an Octokit request error', async () => {
    ...
    const task = pipe(getIssueCommentsPage(args), Effect.flip);
    const result = await Effect.runPromise(task);
  ...
  it('should fail if an api rate limit error', async () => {
    ...
    const task = pipe(getIssueCommentsPage(args), ConsoleTestLayer);
