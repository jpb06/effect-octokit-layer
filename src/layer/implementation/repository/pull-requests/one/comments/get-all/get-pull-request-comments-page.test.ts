...
import type { GetPullRequestCommentsPageArgs } from './get-pull-request-comments-page.js';

vi.mock('@octokit/core');

describe('getPullRequestCommentsPage effect', () => {
  const args: GetPullRequestCommentsPageArgs = {
    owner: 'cool',
    repo: 'story bro',
    pullNumber: 1,
    page: 1,
  ...
  it('should retun data with links', async () => {
    ...
    const { getPullRequestCommentsPage } = await import(
      './get-pull-request-comments-page.js'
    );

    const task = getPullRequestCommentsPage(args);
  ...
  it('should fail with an Octokit request error', async () => {
    ...
    const task = pipe(getPullRequestCommentsPage(args), Effect.flip);
    const result = await Effect.runPromise(task);
  ...
  it('should fail if an api rate limit error', async () => {
    ...
    const task = pipe(getPullRequestCommentsPage(args), ConsoleTestLayer);
