...
import type { GetRepoPullRequestsCommentsArgs } from './get-repo-pulls-requests-comments.js';

vi.mock('@octokit/core');

describe('getRepoPullRequestsComments effect', () => {
  const args: GetRepoPullRequestsCommentsArgs = {
    owner: 'cool',
    repo: 'yolo',
  };
  ...
  it('should retun multiple pages data', async () => {
    ...
    const task = getRepoPullRequestsComments(args);
  ...
  it('should fail when one request fails', async () => {
    ...
    const task = pipe(getRepoPullRequestsComments(args), Effect.flip);
    const result = await Effect.runPromise(task);
