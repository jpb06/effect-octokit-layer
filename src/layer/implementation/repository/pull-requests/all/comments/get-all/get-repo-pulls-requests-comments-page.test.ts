...
import type { GetRepoPullRequestsCommentsPageArgs } from './get-repo-pulls-requests-comments-page.js';

vi.mock('@octokit/core');

describe('getRepoPullRequestsCommentsPage effect', () => {
  const args: GetRepoPullRequestsCommentsPageArgs = {
    owner: 'cool',
    repo: 'story bro',
    page: 1,
  };
  ...
  it('should return data with links', async () => {
    ...
    const { getRepoPullRequestsCommentsPage } = await import(
      './get-repo-pulls-requests-comments-page.js'
    );

    const task = getRepoPullRequestsCommentsPage(args);
  ...
  it('should fail with an Octokit request error', async () => {
    ...
    const task = pipe(getRepoPullRequestsCommentsPage(args), Effect.flip);
    const result = await Effect.runPromise(task);
  ...
  it('should fail if an api rate limit error', async () => {
    ...
    const task = pipe(getRepoPullRequestsCommentsPage(args), ConsoleTestLayer);
  ...
  it('should retry one time and then succeed', async () => {
    ...
    const result = await Effect.runPromise(pipe(effect, ConsoleTestLayer));
